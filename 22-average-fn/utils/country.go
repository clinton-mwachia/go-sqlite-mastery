...
import (
	...
	"github.com/clinton-mwachia/go-sqlite-mastery/22-avg-fn/models"
...
func ReadCSV(filename string) ([]models.Country, error) {
	...
	for _, record := range records[1:] { // Skip header row
		...
		area, err := strconv.ParseFloat(record[2], 64)
		if err != nil {
			return nil, err
		}
...
func GetAveragePopulation(db *sql.DB) (float64, error) {
	query := `SELECT AVG(population) FROM countries`
	var avg float64
