...
import (
	...
	"github.com/clinton-mwachia/go-sqlite-mastery/20-glob/models"
...
// FindByNameGlob finds countries whose names match a GLOB pattern.
func FindByNameGlob(db *sql.DB, pattern string) ([]models.Country, error) {
	query := `SELECT id, name, population, area FROM countries WHERE name GLOB ?`
